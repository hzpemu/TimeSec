...
namespace _05final_object
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.checkBox1.Text = "執行任務完成後是否自動關閉本程式";
			// 
			// checkBox2
			// 
...
//			}
            ...
            if(this.dateTimePicker1.Value.Date==System.DateTime.Now.Date)
			{
				...
				{
					this.timer1.Stop();
					MessageBox.Show("輸入指定的小時或分鐘不正確，請檢查並改正再重新運行本程式！","無效操作");
				}
			}
		}
...
//			}
		...
		private void Form1_Load(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				try
				{
					if(check.Substring(starton+5,check.IndexOf(";",starton)-(starton+5)).Length<1){MessageBox.Show("star含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.starton=check.Substring(starton+5,check.IndexOf(";",starton)-(starton+5));}
				}
				catch{}
				try
				{
					if(check.Substring(dida+5,check.IndexOf(";",dida)-(dida+5)).Length<1){MessageBox.Show("dida含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.dida=check.Substring(dida+5,check.IndexOf(";",dida)-(dida+5));}
				}
				catch{}
				try
				{
					if(check.Substring(hmse+5,check.IndexOf(";",hmse)-(hmse+5)).Length<1){MessageBox.Show("hmse含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.hmse=check.Substring(hmse+5,check.IndexOf(";",hmse)-(hmse+5));}					
				}
				catch{}
				try
				{
					if(check.Substring(year+5,check.IndexOf(";",year)-(year+5)).Length<1){MessageBox.Show("year含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.year=check.Substring(year+5,check.IndexOf(";",year)-(year+5));}
				}
				catch{}
				try
				{
					if(check.Substring(moth+5,check.IndexOf(";",moth)-(moth+5)).Length<1){MessageBox.Show("moth含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.moth=check.Substring(moth+5,check.IndexOf(";",moth)-(moth+5));}
				}
				catch{}
				try
				{
					if(check.Substring(dayy+5,check.IndexOf(";",dayy)-(dayy+5)).Length<1){MessageBox.Show("dayy含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.dayy=check.Substring(dayy+5,check.IndexOf(";",dayy)-(dayy+5));}
				}
				catch{}
				try
				{
					if(check.Substring(hour+5,check.IndexOf(";",hour)-(hour+5)).Length<1){MessageBox.Show("hour含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.hour=check.Substring(hour+5,check.IndexOf(";",hour)-(hour+5));}
				}
				catch{}
				try
				{
					if(check.Substring(mite+5,check.IndexOf(";",mite)-(mite+5)).Length<1){MessageBox.Show("mite含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
					else{this.mite=check.Substring(mite+5,check.IndexOf(";",mite)-(mite+5));}
				}
				catch{}
				try
				{
					string dt =this.year+"年"+this.moth+"月"+this.dayy+"日" + "00:00:00";
				...
				try
				{
					if(check.Substring(djsj+5,check.IndexOf(";",djsj)-(djsj+5)).Length<1){MessageBox.Show("djsj含有不正確的路徑或程式名，確認是否該連接有效（打開cfg.ini查看是否有效）","無效操作");}
